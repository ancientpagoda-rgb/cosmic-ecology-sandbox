...
(async () => {
  ...
  try {
    ...
    assert(result.modules.length === 7 && result.modules.includes('planet.interior-tectonics') && result.modules.includes('ecology.seasonal-resource-fields') && result.modules.at(-1) === 'runtime.procedural-living-planet', `Unexpected root module graph: ${result.modules.join(', ')}`);
