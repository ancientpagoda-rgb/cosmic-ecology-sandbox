import { createSumerianCivilizationSimulation } from './sumerian-civilization-model.js';
...
export function createSumerianCivilizationRuntime({
  ...
  function draw() {
    ...
    if (selected) {
      ...
      const lines = [
        `${selected.name} · ${Math.round(selected.population).toLocaleString()} model persons`,
      ...
      const boxWidth = Math.min(width * 0.48, 390 * (window.devicePixelRatio || 1));
      const lineHeight = Math.max(18, height * 0.026);
      const boxHeight = lines.length * lineHeight + 18;
      context.fillStyle = 'rgba(250,245,230,0.86)';
  ...
  function updateInterface() {
    ...
    if (selected) {
      ...
      if (ui.selected) {
        ui.selected.innerHTML = `<strong>${selected.name}</strong><br>Population ${Math.round(selected.population).toLocaleString()} · food ${(selected.foodRatio * 100).toFixed(0)}% · canal ${(selected.canalHealth * 100).toFixed(0)}% · salinity ${(selected.meanSalinity * 100).toFixed(1)}% · administration ${(selected.administration * 100).toFixed(0)}%`;
      }
    }
    ...
    if (ui.eventLog) {
      const recent = snapshot.transactions.recent.slice(-18).reverse();
